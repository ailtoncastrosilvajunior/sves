...
# Palavra-passe por defeito para desenvolvimento / primeiro arranque; em produção defina SVES_SEED_SERVO_PASSWORD.
...
Servo.where(user_id: nil).where.not(email: nil).find_each do |servo|
  ...
  begin
    ...
    ActiveRecord::Base.transaction do
      u = User.create!(email: email, password: pwd_servos, password_confirmation: pwd_servos)
      servo.update!(user: u)
    end
