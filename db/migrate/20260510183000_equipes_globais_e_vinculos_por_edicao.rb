...
# O vínculo servo ↔ equipa passa a ser sempre por edição (lista de disponíveis = todos os servos;
# ocupação dentro da equipa nesta edição = equipes_servos.edicao_id).
class EquipesGlobaisEVinculosPorEdicao < ActiveRecord::Migration[8.1]
  def up
    ...
    say_with_time "Preencher edicao_id nos vínculos a partir das equipas atuais" do
      execute <<-SQL.squish
    ...
    say_with_time "Unificar registos duplicados de equipas com o mesmo nome (base comum)" do
  ...
  def down
    raise ActiveRecord::IrreversibleMigration,
          "Reverta a partir do backup/restauro ou recrie edição_em_equipa manualmente"
  end
end
